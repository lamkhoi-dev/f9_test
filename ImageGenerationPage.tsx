...
Dựa trên hình ảnh này, hãy giúp tôi tạo ra 15 khung hình (khung hình kịch bản) để tạo hình ảnh.

Tất cả 15 khung hình nên có tỷ lệ khung hình 9:16 và mỗi khung hình nên được trình bày dưới dạng một hình ảnh riêng biệt.

Tất cả các khung hình phải mang cảm giác điện ảnh. Trong số 15 khung hình, cần có:

5 khung hình toàn cảnh
5 khung hình cận cảnh
5 khung hình nghệ thuật
Tất cả các khung hình phải có một người (phải chỉ định rõ ràng nhất quán nhân vật giữa các prompt với nhau - ví dụ một kiến trúc sư hoặc người phụ nữ cụ thể). Người đó nên tương tác với không gian, nhưng trọng tâm vẫn nên là môi trường, với người đó đóng vai trò là yếu tố hỗ trợ.

Nhân vật là một người bất kì (phải chỉ định rõ ràng nhất quán nhân vật giữa các prompt với nhau), mặc một bộ đồ bất kì nhưng phù hợp với ngữ cảnh và không gian. nhân vật xuất hiện rất chân thực và chi tiết, ví dụ:
- Tóc nên có độ bóng nhẹ
- Lỗ chân lông trên mặt nên nhìn thấy được
- Da nên có kết cấu tự nhiên
Các khung hình nên mang cảm giác như ảnh thực tế. Chất lượng vật liệu tổng thể nên được nâng cao, và không gian phải hoàn toàn khớp với hình ảnh tham khảo mà tôi đã cung cấp.

Cuối cùng, hãy kết hợp tất cả các gợi ý thành một đoạn văn có thể sao chép và sử dụng trực tiếp.`;
                } else {
                    prompt = `Analyze carefully the uploaded image of the space.
YOUR TASK:
- DO NOT generate any images.
- ONLY generate 15 complete prompts describing camera angles, ready to be used to generate images.
MANDATORY REQUIREMENTS (HARD LOCK):
- Do NOT change the architectural layout, spatial form, or positions of walls, ceilings, floors, doors.
- Do NOT move, add, remove, or redesign any furniture or decor.
- Do NOT change materials, colors, textures, lighting, design style.
- Maintain original scale, dimensions and spatial relations.
- Only change camera angles, positions, height, focal length, and framing.
REQUIREMENTS FOR EACH PROMPT:
- Each prompt must be ONE complete, standalone descriptive paragraph.
- Write with enough detail so it can be used directly for AI image generation.
- Focus on camera angle, viewing direction, composition, and spatial feelings.
- Use professional architectural photography and rendering vocabulary.
- Do NOT refer to other prompts (like "as above" or "same as previous").
- Do NOT add details not present in the original reference.
OUTPUT FORMAT:
- A numbered list from 1 to 15.
- Each number must be a full prompt starting with the phrase "Góc chụp" or "Camera angle".
- Language: English.

Based on this image, please help me create 15 frames (script frames) to generate images.

All 15 frames should have a 9:16 aspect ratio and each frame should be described as a separate prompt.

All frames must have a cinematic feel. Out of the 15 frames, there must be:
- 5 wide/establishing frames
- 5 close-up frames
- 5 artistic frames

All frames must include a person (must clearly and consistently specify the exact person/character across all prompts). The person should interact with the space, but the main focus should still be the environment, with the person serving as a supporting/scale element.

The character is any specific person (must clearly and consistently specify the exact character across prompts, e.g. a female architect in a minimal gray suit or a male designer), wearing any appropriate suit for the context. This character must appear very realistic and detailed, for example:
- Hair should have slight gloss/reflection
- Face pores should be visible
- Skin should have natural texture

The frames should look hyper-realistic. The overall material quality should be enhanced, and the space must fully match the reference image provided.

Finally, combine all the suggestions into one copiable paragraph suitable for direct use.`;
                }
            }
