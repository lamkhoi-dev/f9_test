...
const ApiKeyModal: React.FC = () => {
  ...
  const getProjectId = () => {
    ...
    try {
      ...
      return parsed.project_id || 'Service Account JSON';
    } catch {
      return 'Service Account JSON';
    }
  };
  const maskedKey = getProjectId();
      ...
      <div className="relative w-full max-w-md bg-[#1e293b]/95 backdrop-blur-xl border border-slate-600/50 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className={`flex items-center gap-3 p-3 rounded-xl border ${
            ...
            <div className={`w-2.5 h-2.5 rounded-full ${apiKey ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400'}`} />
            <div className="flex-1 min-w-0">
              <p className={`text-sm font-medium ${apiKey ? 'text-emerald-300' : 'text-amber-300'}`}>
                {apiKey 
                  ? (locale === 'vi' ? 'Đang dùng Key cá nhân' : 'Personal Key active') 
                  : (locale === 'vi' ? 'Đang dùng Key hệ thống' : 'Using system key')}
              </p>
              {apiKey && (
                <p className="text-[11px] text-slate-400 font-mono truncate">Project: {maskedKey}</p>
              )}
            </div>
            {apiKey && (
              <button 
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                {locale === 'vi' ? 'Dán nội dung Service Account JSON' : 'Paste Service Account JSON'}
              </label>
              <div className="relative">
                <textarea
                  ...
                  className="w-full bg-[#0f172a] border border-slate-600/50 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 outline-none font-mono transition-all min-h-[140px] resize-y"
                  autoFocus
                />
              </div>
            </div>
          ...
          <div className="text-center pt-2 border-t border-slate-700/50">
            ...
            >
              {locale === 'vi' ? '🔗 Hướng dẫn tạo Service Account JSON từ Google Cloud' : '🔗 How to create a Service Account JSON in Google Cloud'}
            </a>
          </div>
        </div>
      </div>
