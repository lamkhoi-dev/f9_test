  ...
  const handleAiSuggest = async () => {
    ...
    try {
        ...
        const schema = {
            type: Type.OBJECT,
            properties: {
                lightingCondition: { type: Type.STRING, description: "Mô tả điều kiện ánh sáng tổng thể (ví dụ: Ánh sáng tự nhiên dịu)" },
                mainLightSource: { type: Type.STRING, description: "Nguồn sáng chính trong không gian (ví dụ: Cửa sổ lớn)" },
                lightTone: { type: Type.STRING, description: "Tông màu của ánh sáng (ví dụ: 4500K – Ánh sáng trung tính)" },
                lightEffect: { type: Type.STRING, description: "Hiệu ứng ánh sáng đặc biệt (ví dụ: Ánh sáng phản xạ nước)" },
                contrastLevel: { type: Type.STRING, description: "Mức độ tương phản giữa vùng sáng và tối (ví dụ: Trung bình)" },
                lightMood: { type: Type.STRING, description: "Cảm giác, không khí mà ánh sáng tạo ra (ví dụ: Dịu nhẹ)" },
            },
        };
